...
export default {
  ...
  watch: {
    theme(theme) {
      const edge = (theme === 'light' || theme === 'blue') ? '#e2e2e2' : '#324E6B'
      const edgeLabel = (theme === 'light' || theme === 'blue') ? '#01172E' : '#F5F5F5'
      const nodeLabel = (theme === 'light' || theme === 'blue') ? '#01172E' : '#F5F5F5'
      const arrow = { path: constants.G6ArrowPath, fill: edge }
      this.curColors = { edge, edgeLabel, nodeLabel, arrow };
      ['关系探索', '对象探索', '相对关系'].forEach(each => {
  ...
  methods: {
    ...
    onMouseenterNode(graph, evt) {
      ...
      graph.getNodes().forEach((node) => {
        graph.clearItemStates(node)
        graph.setItemState(node, 'weakenNode', true)
        const model = node.getModel()
        if (model.type === 'TASK') {
          let img = taskIconGrey
          if (model.taskType === '挑战任务') {
      ...
      graph.setItemState(item, 'highlightNode', true)
      this.highlightTask(graph, item)
      graph.getEdges().forEach((edge) => {
